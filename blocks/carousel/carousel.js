 ...
 */
function disableInfiniteSlide(slide) {
  const block = slide.closest('.carousel.one-per-slide');
  if (!block) return;

  const slideIndex = parseInt(slide.dataset.slideIndex, 10);
  const rows = block.querySelectorAll('.carousel-slides > li');
  const lastSlide = rows.length - 1;

  const navButtons = block.querySelectorAll('.carousel-navigation-buttons > button');
  navButtons[0].disabled = (slideIndex === 0);
  navButtons[1].disabled = (slideIndex === lastSlide);
}
  ...
  const slideObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        disableInfiniteSlide(entry.target);
      }
    });
  }, { threshold: 0.5 });
